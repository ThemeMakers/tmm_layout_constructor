var TMM_ADVANCED_WP_POPUP3 = function() {
	var self = {
		...
		init: function() {
			...
			//***
			jQuery(document.body).on('click', '.advanced_wp_popup_close3', function() {
		...
		popup: function(params) {
			...
			jQuery('#tmm_advanced_wp_popup3').append('<li class="tmm_advanced_wp_popup_li" style="width:' + params.width + 'px; margin-left:-' + params.width / 2 + 'px;">');
		...
		overlay: function(mode, zindex) {
			if (mode) {
				jQuery('#advanced_wp_popup_overlay.advanced_wp_popup_overlay_lc').show();
			} else {
				jQuery('#advanced_wp_popup_overlay.advanced_wp_popup_overlay_lc').hide();
			}
		},
